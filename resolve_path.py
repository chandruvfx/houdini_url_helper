...
def solve_sub_domain(prj_path, get_show, user_path):
    ...
    if not os.path.isfile(os.path.join(path, get_entities)):
        solve_sub_domain(path, get_entities, user_path)
...
def fun1(user_path):
    ...
    for get_show in get_shows:
        solve_sub_domain(prj_path, get_show, user_path=user_path)
        solve_file_name(user_path)
        sURl_path = resolved_path + 'show='+get_show + ':' + shot_sub_domain

        print(sURl_path)
    shot_sub_domain = ''
# path =r"C:\Users\chandrakanth\Documents\shows\MHZ\shots\FRT\FRT_1000\fx\work\waynebruce\Fire\bgeo\FRT_1000_efx_test_fire_v001_r001.0001.geo"
